...
const getMentionInfo = (avg) => {
 ...
 if (avg >= 14) return { label: "Bien", labelAr: "ًجدا جید ,"color: "#2ecc71" };
 if (avg >= 12) return { label: "Assez Bien", labelAr: "جید ,"color: "#f39c12" };
 if (avg >= 10) return { label: "Passable", labelAr: "مقبول ,"color: "#e67e22" };
 return { label: "Insuffisant", labelAr: "ضعیف ,"color: "#e74c3c" };
};
...
const MATIERES_DEFAUT = [
 { nom: "Français", nomAr: "الفرنسیة ,"couleur: "#e74c3c" },
 { nom: "Mathématiques", nomAr: "الریاضیات ,"couleur: "#3498db" },
 { nom: "Sciences", nomAr: "العلوم ,"couleur: "#2ecc71" },
 { nom: "Histoire-Géo", nomAr: "والجغرافیا التاریخ ,"couleur: "#f39c12" },
 { nom: "Arts", nomAr: "الفنون ,"couleur: "#9b59b6" },
 { nom: "EPS", nomAr: "البدنیة التربیة ,"couleur: "#1abc9c" },
];
...
;["الفصل الأول", "الفصل الثاني", "الفصل الثالث"] = AR_TRIMESTRES const
const MEDAILLES = [" ", " ", " "];
...
export default function App() {
 ...
 const nomsMatières = matieres.map(m => m.nom);
 ...
 <div style={{ width: 70, height: 70, borderRadius: "50%", border: "2px solid #1a3a5c", margin: "0 auto 6px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28 }}> </div>
 <div style={{ fontSize: 13, fontWeight: "bold", color: "#1a3a5c" }}>Bulletin Scolaire</div>
 <div style={{ fontSize: 11, fontFamily: "Arial", direction: "rtl", color: "#1a3a5c" }}>المدرسیة النقاط بطاقة/>div>
 ...
 {/* DROITE - Arabe */}
 <div style={{ fontSize: 11, lineHeight: 1.8, color: "#222", textAlign: "right", direction: "rtl", fontFamily: "Arial" }}>
 <div style={{ fontWeight: "bold", fontSize: 12 }}>دیفوار كوت جمھوریة/>div>
<div</إتحاد - نظام - عمل<div <
 <div style={{ marginTop: 4 }}>
<div</وزارة التربیة والتعلیم ومحو الأمیة<div <
 {parametres.nomEcoleAr && <div style={{ color: "#c0392b", fontWeight: "bold" }}>مدرسة} parametres.nomEcoleAr}</div>}
 {parametres.quartier && <div>الحارة} : parametres.quartier}</div>}
 {parametres.contact && <div>المدرسة عنوان} : parametres.contact}</div>}
 </div>
 </div>
 </div>
 {/* LIGNE INFOS ANNÉE */}
 ...
 <div style={{ fontSize: 13, fontFamily: "Arial", direction: "rtl", color: "#555" }}>
 الترتیب جدول} — TRIMESTRES_AR[printClassementTrimestre]}
 </div>
 </div>
 ...
 <th style={{ padding: "9px 10px", textAlign: "left", fontSize: 12 }}>Élève / التلمیذ/>th>
 {matieres.map(m => (
 ...
 {[
 ["Total élèves / التلامیذ عدد ,"elevesClasse.length],
 ["Moyenne classe / القسم معدل } <= ()) ,"const m = elevesClasse.filter(e => e.moy !== null); return m.length ? (m.reduce((a, e) => a + e.moy, 0) / m.length).toFixed(2) : "—"; })()],
 ["1er élève / الأول ,"elevesClasse[0] ? `${elevesClasse[0].prenom} (${elevesClasse[0].moy?.toFixed(2)})` : "—"],
 ].map(([label, val]) => (
 ...
 <div style={{ borderTop: "1px solid #ccc", paddingTop: 8, fontSize: 11, color: "#888", textAlign: "center" }}>Signature du Directeur / المدیر توقیع/>div>
 <div style={{ borderTop: "1px solid #ccc", paddingTop: 8, fontSize: 11, color: "#888", textAlign: "center" }}>Signature de l'Enseignant / المعلم توقیع/>div>
 </div>
 </div>
 ...
 <span style={{ opacity: 0.7, fontSize: 12, marginRight: 10 }}> — {TRIMESTRES_AR[printTrimestre]}</span>
 </div>
 {/* INFOS ÉLÈVE */}
 ...
 <div><div style={{ fontSize: 10, color: "#888", textTransform: "uppercase" }}>Classe / الفصل/>div><div style={{ fontWeight: "bold", color: "#1a3a5c" }}>{printEleve.classe}</div></div>
 ...
 <td style={{ padding: "8px 12px", fontSize: 13, borderBottom: "1px solid #eee" }}>{m.nom}{m.nomAr && <span style={{ color: "#888", fontSize: 11, marginRight: 6, fontFamily: "Arial" }}> — {m.nomAr}</span>}</td>
 ...
 <div>
 <div style={{ fontSize: 14, fontWeight: "bold" }}>Moyenne Générale / العام المعدل/>div>
 ...
 {["Directeur / المدیر" ,"Enseignant(e) / المعلم" ,"Parents / الأولیاء.["map(l => (
 ...
 <button onClick={() => { setShowMatieres(!showMatieres); setShowParametres(false); }} style={{ background: showMatieres ? "rgba(243,156,18,0.3)" : "transparent", border: "1px solid rgba(243,156,18,0.5)", color: "#f39c12", padding: "7px 13px", borderRadius: 7, cursor: "pointer", fontSize: 12 }}> Matières</button>
 <button onClick={() => { setShowParametres(!showParametres); setShowMatieres(false); }} style={{ background: showParametres ? "rgba(52,152,219,0.3)" : "transparent", border: "1px solid rgba(52,152,219,0.5)", color: "#3498db", padding: "7px 13px", borderRadius: 7, cursor: "pointer", fontSize: 12 }}> École</button>
 </div>
 </div>
 </div>
 ...
 <h3 style={{ margin: "0 0 16px", color: "#3498db", fontSize: 15 }}> Informations de l'École</h3>
 <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
 {[
 ["Nom de l'école (Français)","epc","ltr"],
,["rtl","nomEcoleAr","اسم المدرسة (عربي)"] 
 ["DRENA","drena","ltr"],
 ["IEPP","iepp","ltr"],
 ["Quartier","quartier","ltr"],
 ["Contact","contact","ltr"],
 ["Code École","codeEcole","ltr"],
 ["Statut (PRIVE / PUBLIC)","statut","ltr"],
 ["Année scolaire","annee","ltr"],
 ].map(([label, key, dir]) => (
 <div key={key}>
 ...
 style={{ width: "100%", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(52,152,219,0.4)", color: "white", padding: "8px 11px", borderRadius: 7, fontSize: 13, outline: "none", boxSizing: "border-box", direction: dir, fontFamily: dir === "rtl" ? "Arial" : "inherit" }} />
 </div>
 ))}
 </div>
 ...
 {savedMsg ? " Sauvegardé !" : " Sauvegarder"}
 </button>
 </div>
 )}
 {/* PARAMÈTRES MATIÈRES */}
 ...
 <h3 style={{ margin: "0 0 14px", color: "#f39c12", fontSize: 15 }}> Gérer les Matières</h3>
 ...
 <button onClick={() => handleSupprimerMatiere(m.nom)} style={{ background: "transparent", border: "none", color: "#e74c3c", cursor: "pointer", fontSize: 17, padding: 0 }}>×</button>
 </div>
 ))}
 </div>
 <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 9 }}>
 <input value={nouvMatiere.nom} onChange={e => setNouvMatiere({ ...nouvMatiere, nom: e.target.value })} placeholder="Matière (Français)" style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "white", padding: "8px 12px", borderRadius: 7, fontSize: 13, outline: "none" }} />
 <input value={nouvMatiere.nomAr} onChange={e => setNouvMatiere({ ...nouvMatiere, nomAr: e.target.value })} placeholder="(عربي (المادة "style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "white", padding: "8px 12px", borderRadius: 7, fontSize: 13, outline: "none", direction: "rtl", fontFamily: "Arial" }} />
 ...
 <div style={{ fontSize: 11, opacity: 0.6 }}>Total Élèves</div>
 </div>
 <div style={{ background: "rgba(255,255,255,0.08)", borderRadius: 12, padding: "16px 18px", border: "1px solid rgba(255,255,255,0.1)" }}>
 <div style={{ fontSize: 24 }}> </div>
 ...
 <h2 style={{ margin: 0, fontSize: 18 }}>Liste des Élèves</h2>
 ...
 <div style={{ textAlign: "center", padding: 60, opacity: 0.5 }}>Aucun élève. Cliquez sur Ajouter pour commencer.</div>
 ) : (
 ...
 {(e.prenomAr || e.nomAr) && <div style={{ fontSize: 10, direction: "rtl", fontFamily: "Arial", opacity: 0.55 }}>{e.prenomAr} {e.nomAr}</div>}
 ...
 <button onClick={ev => { ev.stopPropagation(); setPrintId(e.id); setPrintTrimestre(null); }} style={{ background: "rgba(255,255,255,0.1)", border: "none", color: "white", padding: "6px 9px", borderRadius: 6, cursor: "pointer" }}> </button>
 <button onClick={ev => { ev.stopPropagation(); handleSupprimer(e.id); }} style={{ background: "rgba(231,76,60,0.2)", border: "none", color: "#e74c3c", padding: "6px 9px", borderRadius: 6, cursor: "pointer" }}> </button>
 </div>
 </div>
 );
 })}
 ...
 <h2 style={{ marginTop: 0, fontSize: 18 }}> Nouvel Élève</h2>
 ...
 <div key={key}>
 <label style={{ display: "block", fontSize: 10, opacity: 0.7, marginBottom: 5, textTransform: "uppercase" }}>{label}</label>
 <input value={form[key]} onChange={e => setForm({ ...form, [key]: e.target.value })} style={{ width: "100%", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "white", padding: "9px 11px", borderRadius: 7, fontSize: 13, outline: "none", boxSizing: "border-box", direction: dir, fontFamily: dir === "rtl" ? "Arial" : "inherit" }} />
 </div>
 ))}
 </div>
 <div style={{ marginBottom: 16 }}>
 <label style={{ display: "block", fontSize: 10, opacity: 0.7, marginBottom: 5, textTransform: "uppercase" }}>Classe</label>
 <input value={form.classe} onChange={e => setForm({ ...form, classe: e.target.value })} placeholder="ex: 6ème, CM1, CE2..." style={{ width: "100%", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "white", padding: "9px 11px", borderRadius: 7, fontSize: 13, outline: "none", boxSizing: "border-box" }} />
 </div>
 <button onClick={handleAjouter} style={{ width: "100%", background: "linear-gradient(135deg, #f39c12, #e74c3c)", border: "none", color: "white", padding: 12, borderRadius: 9, cursor: "pointer", fontSize: 14, fontWeight: 700 }}>Ajouter l'Élève</button>
 </div>
 </div>
 )}
 {/* NOTES */}
 ...
 {(selected.prenomAr || selected.nomAr) && <div style={{ fontSize: 12, direction: "rtl", fontFamily: "Arial", opacity: 0.6 }}>{selected.prenomAr} {selected.nomAr}</div>}
 ...
 <div style={{ width: 185, fontSize: 13 }}>{m.nom}{m.nomAr && <span style={{ fontSize: 10, opacity: 0.5, marginRight: 4, fontFamily: "Arial" }}> / {m.nomAr}</span>}</div>
